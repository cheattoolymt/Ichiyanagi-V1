...
現行の V1 実装は 2 経路を持つ。`interp.c` は AST ツリーウォーク評価器であり、上記パイプラインのうち [1][2] を経て AST を直接実行する（全機能対応・全 OS / 全アーキテクチャ動作）。加えて [3] の `codegen.c` を実装済みであり、AST から x86-64 アセンブリ（System V AMD64 ABI、GAS AT&T 記法）を自前生成し、`as` / `ld` に引き渡して libc 非依存の静的 ELF 実行ファイルを生成する（`ichiyanagi --compile file.ich -o out`）。

`codegen.c` が V1 でコンパイル対象とする言語サブセットは以下の通り。

- 数値（64bit 整数モデル）／真偽値（0/1）
- 算術 `+ - * /`（整数除算）／比較 `gt lt eq neq ge le`／論理 `not and or`（短絡評価）
- `var`／`pvar` の宣言・代入、`var x` によるグローバル値参照
- 関数定義・呼び出し・再帰・`ret`（引数は System V AMD64 の整数レジスタ 6 本の範囲）
- `if`／`else`／`while`
- `terminal.print`／`terminal.printn`（数値および文字列リテラル）

上記に含まれない構文（`list`／`array` などの動的値、`hold`／`drop`、`pick`／`error`、`terminal` 以外のモジュール呼び出し、非整数リテラル）は V1 の `codegen.c` では非対応であり、`--compile` 時に理由を提示してコンパイルを中止する（該当プログラムはインタプリタ経路で全機能が実行可能）。整数モデルのため、除算・真偽値の表示はインタプリタと差異が出る（例: `10 / 3` はインタプリタでは `3.33333`、コンパイラでは整数除算で `3`）。これらの動的値のネイティブ対応は将来バージョンの拡張課題とする。
