...
Ichiyanagi は、Windows / macOS / Linux（x86-64）でユニバーサルに動作することを目標としたコンパイル言語である。
コンパイラ自体は C で実装し、x86-64 アセンブリを自前生成、各 OS 純正のアセンブラ／リンカに引き渡すことでネイティブ実行ファイルを生成する。

- **実装言語**: C
- **出力**: x86-64 アセンブリ（自前生成）→ 各 OS のアセンブラ/リンカ
- **対応 OS**: Linux / macOS / Windows（x86-64。ARM64 等は将来対応）
...
[2] 構文解析 (Parser)  トークン列 → AST（抽象構文木）
    │
    ▼
[3] コード生成 (Codegen)  AST → x86-64 アセンブリ
    │
    ▼
各OSのアセンブラ/リンカ (as/ld, clang, masm/link.exe 等)
    │
    ▼
ネイティブ実行ファイル
```
...
Makefile と CMake の両対応でプロジェクトを構成する。コンパイラ実装自体は本セッションでは行わず、別途実装フェーズで着手する。
...
x86-64 アセンブリを生成した上で、OS ごとに以下の差異を吸収する必要がある（実装フェーズでの検討課題）。

- **ABI**: Linux/macOS は System V AMD64 ABI、Windows は Microsoft x64 ABI（レジスタ渡し規則が異なる）
- **実行ファイルフォーマット**: Linux は ELF、macOS は Mach-O、Windows は PE-COFF
- **アセンブラ/リンカ**: Linux は `as`/`ld`、macOS は `clang` 経由のアセンブル、Windows は `masm`/`link.exe` 相当

V1 ではまず1 OS（Linux）向けの生成を確立し、その後 macOS・Windows 向けにコード生成部を拡張する方針とする。
